...
class Main_page_col_cell_8: UICollectionViewCell {
    ...
    func init_component(){
        ...
        thanks_imagebutton.addTarget(self, action: #selector(q1), for: .touchUpInside)
        // Setting for helplines_imagebutton
        self.stack_view.addSubview(thanks_imagebutton)
        thanks_imagebutton.topAnchor.constraint(equalTo: stack_view.topAnchor).isActive = true
        thanks_imagebutton.leadingAnchor.constraint(equalTo: stack_view.leadingAnchor).isActive = true
        thanks_imagebutton.heightAnchor.constraint(equalTo: stack_view.heightAnchor).isActive = true
        thanks_imagebutton.widthAnchor.constraint(equalTo: stack_view.widthAnchor).isActive = true
    }
    ...
    let thanks_imagebutton: UIButton = {
        let btn = UIButton()
        btn.translatesAutoresizingMaskIntoConstraints = false
        btn.contentMode = .scaleAspectFit
        btn.setImage(UIImage(named: "merci2"), for: .normal)
        return btn
    }()
