...
class Main_page_col_cell_3: UICollectionViewCell {
    ...
    func init_component(){
        ...
        map_button.addTarget(self, action: #selector(q1), for: .touchUpInside)
        // Setting for helplines_imagebutton
        self.stack_view.addSubview(map_button)
        map_button.topAnchor.constraint(equalTo: stack_view.topAnchor).isActive = true
        map_button.leadingAnchor.constraint(equalTo: stack_view.leadingAnchor).isActive = true
        map_button.heightAnchor.constraint(equalTo: stack_view.heightAnchor).isActive = true
        map_button.widthAnchor.constraint(equalTo: stack_view.widthAnchor).isActive = true
    }
    ...
    let map_button: UIButton = {
        ...
        btn.setImage(UIImage(named: "nc"), for: .normal)
        return btn
    }()
    
    @objc func q1(){
        let nav_view = Map_page_view()
